...
--
-- Client :  127.0.0.1
-- Généré le :  Lun 30 Janvier 2017 à 13:52
...
CREATE TABLE IF NOT EXISTS `client` (
  `Code_c` int(11) NOT NULL AUTO_INCREMENT COMMENT 'Identifiant du client',
  ...
  `telephone` varchar(25) DEFAULT NULL COMMENT 'T?l?phone principal du client',
  PRIMARY KEY (`Code_c`)
...
CREATE TABLE IF NOT EXISTS `commande` (
  ...
  `etat` tinyint(3) UNSIGNED DEFAULT '0' COMMENT '0 stock non actualis? 1 stock MAJ',
  PRIMARY KEY (`numero`),
...
CREATE TABLE IF NOT EXISTS `produit` (
  ...
  `designation` varchar(30) NOT NULL COMMENT 'D?signation du produit',
  `quantite` int(11) DEFAULT '0' COMMENT 'Poids du produit ou nombre de pi?ces',
  `descriptif` varchar(1) NOT NULL DEFAULT 'G' COMMENT 'Unit?de mesure G pour gramme et P pour Pi?ce',
  `prix_unitaire_HT` double DEFAULT '0' COMMENT 'Prix unitaire hors taxes',
  `stock` smallint(6) DEFAULT '0' COMMENT 'Etat du stock',
  `poids_piece` int(11) DEFAULT '0' COMMENT 'Poids d''une pi?ce en grammes pour les articles vendus par pi?ce',
...
CREATE TABLE IF NOT EXISTS `vendeur` (
  `Code_v` int(11) NOT NULL AUTO_INCREMENT COMMENT 'Identifiant du vendeur',
  `nom` varchar(35) NOT NULL COMMENT 'Nom et Prénom',
  `adresse` varchar(50) DEFAULT NULL COMMENT 'Adresse du vendeur',
  `cp` varchar(5) NOT NULL COMMENT 'Code postal',
  `ville` varchar(25) NOT NULL COMMENT 'Ville',
  `telephone` varchar(25) DEFAULT NULL COMMENT 'T?l?phone principal du vendeur',
  PRIMARY KEY (`Code_v`)
...
ALTER TABLE `commande`
  ADD CONSTRAINT `commande_ibfk_1` FOREIGN KEY (`code_c`) REFERENCES `client` (`Code_c`),
  ADD CONSTRAINT `commande_ibfk_2` FOREIGN KEY (`code_v`) REFERENCES `vendeur` (`Code_v`);
